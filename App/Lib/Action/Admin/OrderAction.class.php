...
class OrderAction extends OrderBaseAction {
	...
	private function data_check() {
		...
		if (Validate::checkNull($_POST['over'])) $this->error('用车结束时间为空');
		if (Validate::checkNull($_POST['length'])) $this->error('用车天数为空');
	}
	...
	public function edit_apply () {
		...
		$MemberResource = D('MemberResource');		// 会员可用资源表
		$MemberBase = D('MemberBase');						//会员基本信息表
		$Cars = D('Cars');													//车辆资源表
		$Order = D('Order');												//订单模型表
		
		/* 通过会员ID，找到会员对应的会员信息，以及会员级别 */
		$member_info = $MemberBase->get_one_data(array('status'=>0,'id'=>$member_base_id),'member_rank_id,use_car_number,name');
	...
	public function cars_arrange_edit () {
		...
		if ($this->isPost()) {
			...
			if (parent::send_shp($mobile_phone, $mobile_phone_message) == 0) {
				$phone_mes = '发送失败！';
			} else {
				$phone_mes = '发送成功！';
			}
			
			//派车申请通过，已向123456789123发送短信，发送状态为：发送成功。短信内容为：$mobile_phone_message
			$history_content = $state_content.'。已向'.$mobile_phone.'发送短信,发送状态为：'.$phone_mes.'。短信内容为：'.$mobile_phone_message;
			
			parent::order_history($id,$history_content);
			
			$this->success($state_content.'。短信发送状态为：'.$phone_mes);
			exit;
		}
